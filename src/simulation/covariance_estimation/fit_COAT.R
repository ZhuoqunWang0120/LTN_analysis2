...
cnt=readRDS(paste0(cachedir,'sim_ntop100seed',SEED,'.rds'))$cnt
cnt[cnt==0]=0.5
...
for (modelCov in 1:3){
  ...
  clrtrue=readRDS(paste0(cachedir,modelcov,'clrcov_',i,'ntop100.rds'))
  cnt=readRDS(paste0(cachedir,modelcov,'cnt_',i,'ntop100.rds'))
  cnt[cnt==0]=0.5
  coat_est=coat(cnt)$sigma
